 ...
 * 【4-2】スタッフ行、ダミー予約列を選択
 */
public class AAdjustRowCellCancel implements Command
{
  /** 予約情報 */
  ...
  public void execute()
  {
    ...
    // ダミー予約を取得
    List<WebElement> dummyReserves = orders_cells__container.findElements(By.className("orders-booking"));
    for (WebElement dummyReserve : dummyReserves)
    {
      // 予約番号が異なるものは飛ばす
      if (!dummyReserve.getAttribute("title").contains(order.getReserveId())) continue;
